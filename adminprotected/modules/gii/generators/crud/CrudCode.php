...
class CrudCode extends CCodeModel
{
	...
	public function generateActiveField($modelClass,$column)
	{
		if($column->type==='boolean')
		...
		else if(stripos($column->dbType,'time')!==false || stripos($column->dbType,'data')!==false)
			...
			return "\$form->radioButtonList(\$model,'{$column->name}', $data, array('class'=>'radio'))";
                }
                else
		{
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
			...
			elseif(preg_match('/^(\w+_)?(cat_id|cat|parent_id)$/i',$column->name,$matches))
                        {
				...
				return "\$form->dropDownList(\$model,'{$column->name}',\${$prefix}Categorys)";
			}
			elseif(preg_match('/(file|img|image|photo)$/i',$column->name))
                        {
				return "\$form->fileField(\$model,'{$column->name}')";
			}
			else
