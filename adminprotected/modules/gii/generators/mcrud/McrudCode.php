...
class McrudCode extends CCodeModel
{
	public $tablePrefix;
	public $tableName;
	public $modelClass;
	...
	public function generateActiveField($modelClass,$column,$types)
	{
		...
		elseif(stripos($column->dbType,'text')!==false)
			return "\$form->textAreaRow(\$model,'{$column->name}',array('class'=>'ckeditor', 'id'=>'editor1', 'rows'=>10, 'cols'=>80))";
		elseif(stripos($column->dbType,'time')!==false || stripos($column->dbType,'data')!==false || $types[$column->name] == 'time')
			return "\$form->textFieldRow(\$model,'{$column->name}',array('id'=>'{$column->name}', 'readonly'=>'readonly', 'onclick'=>\"return showCalendar('{$column->name}','%Y-%m-%d',false,false,'{$column->name}');\"))";
		elseif($types[$column->name] == 'pic')
			return "\$form->textFieldRow(\$model,'{$column->name}',array('onclick'=>\"return uploadImage('{$column->name}')))";
		elseif($types[$column->name] == 'desc')
			return "\$form->textAreaRow(\$model,'{$column->name}',array('rows'=>10, 'cols'=>80,'class'=>'span5'))";
		elseif($types[$column->name] == 'select'){
			$varName = preg_match('/^^(.+)_id$/i',$column->name) ? '$' . lcfirst($this->getClassNameByVar($column->name)) . 's' : 'array()';
			return "\$form->dropDownListRow(\$model,'{$column->name}',{$varName})";
			...
			return "\$form->radioButtonListInlineRow(\$model,'{$column->name}',{$modelClass}::all".ucfirst($this->getClassNameByVar($column->name))."())";
		}elseif($types[$column->name] == 'checkbox'){
			$varName = preg_match('/^^(.+)_id$/i',$column->name) ? '$' . lcfirst($this->getClassNameByVar($column->name)) . 's' : 'array()';
			return "\$form->checkBoxListInlineRow(\$model,'{$column->name}',{$modelClass}::all".ucfirst($this->getClassNameByVar($column->name))."())";
		}elseif($types[$column->name] == 'file')
			return "\$form->textFieldRow(\$model,'{$column->name}',array('onclick'=>'checkFile()'))";
		else if(stripos($column->dbType,'enum')!==false)
		{
			...
			return "\$form->radioButtonListInlineRow(\$model,'{$column->name}', $data)";
		}
		else
		{
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
