...
$('#{$class}_tableName').change(function(){
	var tableName=$(this).val();
	// 模块生成选项
	$.post('mcrud/AjaxGetColumns', {tableName:tableName,YII_CSRF_TOKEN:'".Yii::app()->request->csrfToken."'}, function(data){
	...
	<div class="row">
		...
		if(isset($model->tableName) && !empty($model->tableName))
		{
			...
			if(!empty($columns)){
				...
				include(dirname(dirname(dirname(__FILE__))).'/params.php');
			}
		}
		?>
		</div>
